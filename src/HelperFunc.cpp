...
std::string HelperFunc::getTime(unsigned long int seconds){
...
std::string HelperFunc::getProgressBar(std::string percent){
    std::string bar;
    bar.erase();
...
std::ifstream HelperFunc::getStream(std::string path){
    std::ifstream stream(path);                           //stream object for reading into a file.
    if(!stream){
        throw std::runtime_error(errors::invalidPID().c_str()); // runtime error in case the path fails to open       
    }
    return stream;
}